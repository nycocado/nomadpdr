...
export function HeroSection({ dict }: { dict: Dictionary['hero'] }) {
  return (
    ...
    >
      ...
      {/* Navbar Spacer for Visual Centering */}
      <div className="h-16 shrink-0" aria-hidden="true" />

      {/* Centered Content Area */}
      <div className="flex-1 flex items-center w-full relative z-10">
        <div className="container mx-auto px-4 md:px-8 flex flex-col lg:flex-row items-center gap-8 md:gap-12">
          ...
          <div className="flex-1 text-center lg:text-left space-y-6 order-2 lg:order-1">
            <h1 className="hidden lg:block text-4xl md:text-6xl lg:text-7xl font-heading font-extrabold italic text-foreground leading-tight tracking-tight">
              {dict.title_start} <br />
              <span className="text-transparent bg-clip-text bg-[image:var(--gradient-nomad-red)]">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 pt-4">
              <a href="#contact" className="w-full sm:w-auto">
                <Button
                  variant="nomad"
                  size="lg"
                  className="w-full text-base px-8 h-14"
                >
                  {dict.quote_button}
                </Button>
              </a>
              ...
              >
                <Button
                  variant="outline"
                  size="lg"
                  className="w-full h-14 border-primary/20 text-foreground hover:bg-primary/5 bg-transparent"
                >
